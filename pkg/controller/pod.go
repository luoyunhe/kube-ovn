	...
	if !subnet.Spec.UnderlayGateway {
		if vpc.Status.Default {
			...
			if subnet.Spec.GatewayType == kubeovnv1.GWDistributedType {
				...
				if err := c.ovnClient.AddStaticRoute(ovs.PolicySrcIP, podIP, nodeTunlIPAddr[0].String(), c.config.ClusterRouter); err != nil {
					klog.Errorf("failed to add static route, %v", err)
					return err
				}
			}
...
func getNodeTunlIP(node *v1.Node) ([]net.IP, error) {
	...
	v4NodeTunlIP := nodeTunlIP
	v6NodeTunlIP := nodeTunlIP
	...
	if util.IsValidIP(v4NodeTunlIP) && util.IsValidIP(v6NodeTunlIP) {
		nodeTunlIPAddr = append(nodeTunlIPAddr, net.ParseIP(v4NodeTunlIP))
		nodeTunlIPAddr = append(nodeTunlIPAddr, net.ParseIP(v6NodeTunlIP))
	} else if util.IsValidIP(v4NodeTunlIP) {
		nodeTunlIPAddr = append(nodeTunlIPAddr, net.ParseIP(v4NodeTunlIP))
	} else if util.IsValidIP(v6NodeTunlIP) {
		nodeTunlIPAddr = append(nodeTunlIPAddr, net.ParseIP(v6NodeTunlIP))
	} else {
